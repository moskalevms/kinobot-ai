...
Подтверждено: **2026-09-02** (деплой веб-приложения, смена бота).

## Текущее состояние

- Продакшен — VPS с Ubuntu 24 (38.180.228.133), пользователь `kinobot`
  (без sudo, в группе `docker`).
- Продакшен-бот — **@AIEveningReelBot** (токен — в `.env.production`).
- Развёрнутые сервисы (контейнеры):
  - `kinobot` — Telegram-бот (`python src/telegram_bot.py`), образ
    ...
    GitHub Actions;
  - `kinobot-web` — веб-интерфейс и админ-панель (тот же образ,
    `gunicorn` с одним воркером), порт 5000 опубликован наружу;
...
- Секреты: на VPS в `~/kinobot/.env.production` (права 600);
  в репозитории их нет — файл записывает пайплайн из секрета
  GitHub `ENV_PRODUCTION` при каждом деплое.
