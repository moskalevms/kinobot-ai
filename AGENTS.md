...
- `/make-task <ID|P0|P1|P2|all|описание>` — итеративная реализация бэклога по
  ...
  **Сессионная модель**: `autodev` — диспетчер, на каждую задачу (или связанную
  группу) спавнит НОВУЮ сессию opencode — воркера `autodev-worker`
  (`opencode run --agent autodev-worker`, detached, логи в
  `backlog/state/logs/`, отчёты в `backlog/state/reports/`); цикл задачи
  (OpenSpec-спека → реализация → сборка+тесты (ruff/mypy/pytest) → ревью →
  повторные сборка+тесты) воркер выполняет в своей сессии; отметка чекбоксов
  `- [x]`/`✅` после верификации.
- `/resume` (или `/make-task resume`) — возобновление после сбоя (пропал
  интернет, упал MCP, оборвалась сессия, умер воркер): чекпоинт
  `backlog/state/autodev-state.json` (gitignored; схема v2 — с `worker.pid`,
  `queue`, `worker_result`) → живой воркер: присоединиться к поллингу, не
  дублировать; мёртв: перезапустить единицу в новой сессии (`attempt+1` ≤3) →
  сверка с артефактами (артефакты главнее) → продолжение с шага остановки без
  дублей. Преходящие сетевые сбои — ретраи ≤3 с паузами, не тратят бюджет
  исправлений (≤3 на задачу); исчерпаны → статус `interrupted` и отчёт.
- Агенты: `autodev` (диспетчер), `autodev-worker` (одна задача/группа в
  отдельной сессии, сессии не спавнит); сабагенты: `researcher`,
  `backlog-planner`, `implementer`, `verifier` (только проверки, не правит код),
  `reviewer` (только ревью).
- Конвенции: исследования — в `docs/research/`, бэклог — в `backlog/`,
  рантайм-состояние (state, логи и отчёты воркеров) — в `backlog/state/`.
